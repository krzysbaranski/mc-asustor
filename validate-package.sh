...
# Validation script for Midnight Commander ASUSTOR package
# This script checks if all files mentioned in config.json exist
# and warns about unexpected files in bin and lib directories

PACKAGE_DIR="$(pwd)/apkg"
CONFIG_FILE="${PACKAGE_DIR}/CONTROL/config.json"

echo "================================================"
echo "Validating Midnight Commander ASUSTOR Package"
echo "================================================"
echo ""
...
validate_directory() {
    ...
    if [ ! -d "$dir_path" ]; then
        echo "  WARNING: Directory $dir_path does not exist (may not have files for this category)"
        WARNINGS=$((WARNINGS + 1))
        return
    fi
    ...
    if [ $missing_count -eq 0 ] && [ -n "$expected_files" ]; then
        echo "  ✓ All expected files present"
    elif [ -z "$expected_files" ]; then
        echo "  ✓ No files expected in this directory"
    else
        echo "  ✗ $missing_count file(s) missing"
    fi

    # Check for unexpected files (only if we have expected files)
...
if [ $VALIDATION_FAILED -eq 1 ]; then
    ...
    echo "  Some files may be unexpected or directories may be missing."
    echo "  Consider updating config.json if needed."
    exit 0
else
